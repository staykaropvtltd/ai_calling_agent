...
async def test_admin_route_rejects_unauthenticated(
    ...
    assert r.status_code == 401, f"{method} {path} (no token) returned {r.status_code}, expected 401"
...
async def test_admin_route_role_gate(
    ...
    r = await api_client.request(method, path, json=body if body is not None else {}, headers=_headers(role))
...
async def test_tampered_jwt_signature_rejected_on_every_admin_route(api_client: AsyncClient):
    ...
    for method, path, body, _allowed in _ADMIN_ROUTES:
        r = await api_client.request(method, path, json=body if body is not None else {}, headers=headers)
