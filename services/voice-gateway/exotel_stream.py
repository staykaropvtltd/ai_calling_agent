...
def decode_media(payload: str) -> InputAudioRawFrame:
    try:
        ...
        raise ValueError("invalid Exotel media payload") from exc
    if len(audio) % 2:
        raise ValueError("invalid 16-bit PCM payload")
...
def encode_media(stream_sid: str, frame: OutputAudioRawFrame) -> dict:
    if frame.sample_rate != 8000 or frame.num_channels != 1:
        raise ValueError("Exotel requires 8kHz mono PCM")
    ...
    async def stream(ws: WebSocket):
        ...
        async def cleanup(reason: str):
            ...
            if state.call_id:
                ...
                if session_manager.get(state.call_id):
                    session_manager.end(state.call_id)
                    session_manager.remove(state.call_id)
            if handle:
        ...
        try:
            while True:
                ...
                if kind == 'start':
                    ...
                    to = start.get('to')
                    if routing is None or not to:
                        await ws.close(code=1011)
                        return
                    tenant, agent = await routing.resolve(to)
                    state.call_id = str(uuid4())
                    state.stream_sid = start.get('stream_sid') or event.get('stream_sid')
                    ...
                    async def outbound(frame):
                        if state.stream_sid:
                            await ws.send_json(encode_media(state.stream_sid, frame))
                    on_transcript = make_transcript_handler(state.call_id)
                    handle = CallPipelineHandle(ws, state.call_id, outbound, externally_fed=True, stt_provider=stt_provider, on_transcript=on_transcript)
                    await handle.start()
                elif kind == 'media':
                    if handle is None:
                        continue
