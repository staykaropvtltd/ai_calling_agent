...
Coverage:
 ...
 T04  TwilioProvider maps 'sid' ΓåÆ provider_call_id on success
 T05  TwilioProvider HTTP error (4xx) raises ProviderError("rejected")
 T06  TwilioProvider timeout raises ProviderError("timed out")
 T07  Webhook missing X-Twilio-Signature header ΓåÆ 401
 T08  Webhook with wrong signature value ΓåÆ 401
 T09  Malformed payload missing CallSid ΓåÆ 422 (valid signature, missing field)
 T10  Duplicate (CallSid, CallStatus) event ΓåÆ returns "duplicate", no second create()
 T11  CallStatus=in-progress ΓåÆ session_started, calls.create() called with correct tenant/agent
 T12  CallStatus=completed ΓåÆ session_cleaned, calls.finalize() called with end_reason=caller_hangup
 T13  CallStatus=failed ΓåÆ end_reason=provider_failure
...
# Fixed test credentials ΓÇö no webhook_secret; auth_token is used for signing.
...
# ---------------------------------------------------------------------------
# T03  Request construction ΓÇö form body and auth
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T07  Webhook auth ΓÇö missing header
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T08  Webhook auth ΓÇö wrong signature (not the correct HMAC-SHA1)
# ---------------------------------------------------------------------------
...
def test_T08_webhook_wrong_signature_returns_401():
    ...
    resp = client.post(
        "/telephony/twilio/callback",
        # Deliberately wrong ΓÇö not the HMAC-SHA1 of the form params
...
# ---------------------------------------------------------------------------
# T09  Malformed payload ΓÇö valid signature but missing CallSid ΓåÆ 422
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T11  Start on in-progress ΓÇö full lifecycle
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T12  Finalize on completed ΓÇö caller_hangup
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T13  Finalize on failed ΓÇö provider_failure
# ---------------------------------------------------------------------------
